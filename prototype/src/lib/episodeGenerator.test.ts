...
describe("generateEpisodeDraft", () => {
  ...
  it("adds review safeguards for risky publishing claims", () => {
    const draft = generateEpisodeDraft(defaultCharacter, vlogTemplates[2], {
      platform: "instagram",
      tone: "direct",
      ctaGoal: "follow",
      lengthSeconds: 20,
    ...
    expect(draft.reviewChecklist.join(" ")).toContain("No claim");
    expect(draft.reviewChecklist.join(" ")).toContain("Human review");
  });
});
