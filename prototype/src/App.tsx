...
interface SavedEpisodeDraft extends EpisodeDraft {
  savedAt: string;
...
const metricFields: Array<[keyof Omit<MetricRecord, "draftId" | "title" | "platform" | "notes">, string]> = [
  ["views", "Views"],
  ["likes", "Likes"],
  ["comments", "Comments"],
  ["followers", "Followers"],
    ...
    <div className="app-shell">
        ...
        <div className="field-stack">
          <label>
            ...
            <input
              ...
              onChange={(event) => updateCharacter("displayName", event.target.value)}
            />
          </label>
          <label>
            Disclosure
            <input
              value={character.disclosure}
              onChange={(event) => updateCharacter("disclosure", event.target.value)}
            />
          </label>
          <label>
            Public bio
            <textarea
              value={character.publicBio}
              onChange={(event) => updateCharacter("publicBio", event.target.value)}
              rows={4}
            />
          </label>
          <label>
            ...
            <textarea
              ...
              onChange={(event) => updateCharacter("voiceTone", event.target.value)}
              rows={3}
            />
          </label>
          <label>
            ...
            <textarea
              ...
              onChange={(event) => updateCharacter("visualStyle", event.target.value)}
              rows={3}
            />
          </label>
          <label>
      ...
      <main className="workspace">
        <header className="topbar">
          <div>
            <p className="eyebrow">Creator Workflow MVP</p>
      ...
      <section className="panel">
        ...
        <div className="option-row">
          ...
          <label>
            ...
            <input
              type="date"
              value={options.calendarSlot}
              onChange={(event) => updateOptions("calendarSlot", event.target.value)}
            />
          </label>
        </div>
...
function OutputBlock({ draft }: { draft: EpisodeDraft }) {
  return (
    <div className="output-grid">
      <section className="output-section wide">
      ...
      <section className="output-section">
        ...
        <ul className="check-list">
          {draft.publishingChecklist.map((item) => (
            <li key={item}>
              <Upload size={16} />
              {item}
            </li>
...
function QueueView({ savedDrafts, approveDraft, removeDraft, downloadDraft }: QueueViewProps) {
...
function AnalyticsView({ savedDrafts, metrics, setMetrics, totalViews, totalPaidClicks }: AnalyticsViewProps) {
  ...
  const averageEngagement =
    metrics.length === 0
      ? 0
      : metrics.reduce((sum, item) => sum + item.likes + item.comments, 0) / Math.max(1, metrics.length);
